function hyperbolic_cross_derivative(weights::Array{T,1},node::Array{T,1},multi_index::Array{S,2},pos::S) where {T<:AbstractFloat,S<:Integer}
  
    n = 2*maximum(multi_index,dims=1).+1
    d = length(n) # d is the number of dimensions
      
    base_polynomials                   = Array{Array{T,2},1}(undef,d)
    base_polynomial_derivatives        = Array{Array{T,2},1}(undef,d)
    unique_base_polynomials            = Array{Array{T,1},2}(undef,size(multi_index))
    unique_base_polynomial_derivatives = Array{Array{T,1},2}(undef,size(multi_index))
  ...
  function hyperbolic_cross_derivative(weights::Array{T,1},node::Array{T,1},multi_index::Array{S,2},domain::Union{Array{T,1},Array{T,2}},pos::S) where {T<:AbstractFloat,S<:Integer}
